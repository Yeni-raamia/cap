...
export function DistinctionsTab() {
  ...
  const [selId, setSelId] = useState(me.id);
  ...
  const selName = profiles.find((p) => p.id === selId)?.nom ?? "";
  const selBadges = badgesOf(selId);
  ...
  return (
    <div className="space-y-5">
      <GrcTabHeader
        ...
        subtitle="Badges honorifiques décernés à partir de l'activité réelle de l'équipe : rondes, écarts, risques, politiques, actions…"
      />

      {/* Classement d'équipe */}
      <Card className="p-4">
        <div className="text-[13px] font-semibold text-slate-700 mb-3 flex items-center gap-2"><Trophy size={15} className="text-amber-500" /> Classement des distinctions</div>
        <div className="space-y-1.5">
          {board.map(({ p, badges, earned }, i) => (
            <button key={p.id} onClick={() => setSelId(p.id)} className={`w-full text-left flex items-center gap-3 rounded-xl px-2.5 py-2 border transition-colors ${selId === p.id ? "border-emerald-300 bg-emerald-50/50 dark:bg-emerald-500/10" : "border-transparent hover:bg-slate-50 dark:hover:bg-slate-800/50"}`}>
              ...
              <span className="shrink-0 inline-flex items-center gap-1 text-[12px] font-semibold text-slate-500"><Medal size={13} className="text-amber-500" /> {earned}/{selBadges.length}</span>
            </button>
          ))}
        </div>
      </Card>
      ...
      <Card className="p-4">
        ...
        <div className="text-[11px] text-slate-400 mb-3">{selEarned} badge{selEarned > 1 ? "s" : ""} obtenu{selEarned > 1 ? "s" : ""} sur {selBadges.length}</div>
